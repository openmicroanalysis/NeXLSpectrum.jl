...
function isemsa(io::IO)
    for (lx, line) in enumerate(eachline(io))
        ...
        if isnothing(tmp) || #
           (lx == 1 && !(tmp[1] == "FORMAT" && startswith(uppercase(tmp[2]), "EMSA/MAS"))) || #
...
function readEMSA(f::IO, T::Type{<:Real}=Float64)::Spectrum
    ...
    for (lx, line) in enumerate(eachline(f))
        if (lx ≤ 2)
            ...
            if isnothing(res) || ((lx == 1) && (res[1] != "FORMAT" || !startswith(uppercase(res[2]), "EMSA/MAS")))
        ...
        elseif startswith(line, "#")
            try
                ...
                if !isnothing(res)
                    ...
                    elseif key == "COMMENT"
                        if length(value) > 0
                            prev = get(props, :Comment, missing)
                            props[:Comment] = ismissing(prev) ? value : "$prev\n$value"
                        end
                    elseif key == "DATATYPE"
...
function writeEMSA(io::IOStream, spec::Spectrum)
    ...
    haskey(spec, :Owner) && writeline(io, "OWNER", spec[:Owner])
    ...
    haskey(spec, :LiveTime) && writeline(io, "LIVETIME", "$(spec[:LiveTime])")
    ...
    haskey(spec, :Composition) && writeline(io, "#D2STDCMP", todtsa2comp(spec[:Composition]))
    haskey(spec, :Specimen) && writeline(io, "#SPECIMEN", "$(spec[:Specimen])")
